...
====================================================================';
	...
	Host='raw.githubusercontent.com/qingshenyijiu/laoqiuml/master';
...
kcard="${card}-${IP}";nginxcard="${card}-${IP}";httpdcard="${card}-${IP}";
if [[ "$kcard" == "no" ]] || [ "$kcard" == "" ]
then
echo -e '秘钥错误！请检查授权码是否输入正确！授权码从购买日起7天后过期！[高级模式：\033[31m  未开启  \033[0m]'
echo
echo "高级秘钥：6.66 元/7天"
echo "购买请到康师傅免流网站购买:buy.kangml.com"
echo "目前各大VPS测试成功，欢迎大家选购搭建，一瓶康师傅泡面的价格，谢谢"
exit 0;
else
